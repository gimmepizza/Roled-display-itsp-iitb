...
Mat pixellate(Mat pixels)
{
	...
	// The number of colours are also decreased from 255^3 to 1000 to reduce amount of data
	for(int i=0;i<no_of_sectors;i++)
	{
		for(int j=0;j<no_of_leds;j++)
		{
    		...
    		// Further averaging out the color to reduce data
    		averaged_pixels[i][j][0]=min((int)(averaged_pixels[i][j][0]/25)*25+13,255); // over-averaged blue color
    		averaged_pixels[i][j][1]=min((int)(averaged_pixels[i][j][1]/25)*25+13,255); // over-averaged green color
    		averaged_pixels[i][j][2]=min((int)(averaged_pixels[i][j][2]/25)*25+13,255); // over-averaged red color
    	}
    }
...
{
	if(!videoStorageStarted) videoStorageStarted=true;
	...
	switch(displayMode)
	{
		case 's': 
		{
			ofstream fout("data_rgb.txt"); // static mode implies the file has to be overwritten
			fout<<"[";
			for(int i=0;i<no_of_sectors;i++)
			{
				for(int j=0;j<no_of_leds;j++)
				{
					fout<<alter(averaged_pixels[i][j][2])<<alter(averaged_pixels[i][j][1])<<alter(averaged_pixels[i][j][0]);
				}
		...
		{
			ofstream fout;
			if(videoStorageStarted)
				fout.open("data_rgb.txt",ios::app);
			else
				fout.open("data_rgb.txt");
			...
			for(int i=0;i<no_of_sectors;i++)
			{
				...
				for(int j=0;j<no_of_leds;j++)
				{
					fout<<"("<<averaged_pixels[i][j][2]<<" "<<averaged_pixels[i][j][1]<<" "<<averaged_pixels[i][j][0]<<")";
				}
				fout<<"}";
			}
			fout<<"]";
...
int alter(int val)
{
	// val is a number between 0-255
	return min((int)(val/25),9);
}
